...
let completedStudyTimeLessons = []; // Cache of completed lessons with study time

// Study time tracking for re-study mode
let isReStudyMode = false; // Whether we're in re-study mode (all lessons with study time completed)
...
async function findFirstUnfinishedLesson() {
    try {
        ...
        // enter re-study mode for all completed lessons with study time
        debugLog(1, 'No unfinished lessons with study time found at all, entering re-study mode for completed lessons with study time');
        ...
        if (completedLessonsWithStudyTime.length > 0) {
            ...
            if (JSON.stringify(completedLessonsWithStudyTime.map(l => l.displayName)) !== 
                JSON.stringify(completedStudyTimeLessons.map(l => l.displayName))) {
                completedStudyTimeLessons = completedLessonsWithStudyTime;
                completedStudyTimeLessonIndex = 0;
                debugLog(4, 'Updated completed study time lessons cache for re-study mode');
            }
            
            // Select the next lesson in re-study rotation
            const selectedLesson = completedStudyTimeLessons[completedStudyTimeLessonIndex];
            debugLog(1, `Selected completed lesson for re-study (${completedStudyTimeLessonIndex + 1}/${completedStudyTimeLessons.length}):`, selectedLesson);
            ...
            debugLog(1, `Starting re-study of lesson "${selectedLesson.displayName}" with target study time: ${selectedLesson.studyTime} seconds`);
...
function advanceReStudyLesson() {
    ...
    // Move to next lesson in re-study rotation
