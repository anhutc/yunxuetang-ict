...
// Helper function to debug DOM structure
function debugContainerStructure(container, lessonName) {
    debugLog(4, `=== DOM Structure Debug for "${lessonName}" ===`);
    debugLog(4, 'Container classes:', container.className);
    debugLog(4, 'Container children count:', container.children.length);
    ...
    debugLog(4, `Total elements in container: ${allElements.length}`);
    
    // Find elements with time-related text
    const timeElements = [];
    allElements.forEach((el, index) => {
        const text = el.textContent.trim();
        if (text && (text.includes('分钟') || text.includes('min') || text.includes(':') || /\d+/.test(text))) {
            timeElements.push({
                index: index,
                tagName: el.tagName,
    ...
    if (timeElements.length > 0) {
        debugLog(4, 'Elements with potential time content:', timeElements);
    } else {
        debugLog(4, 'No elements found with time-related content');
    }
    
    debugLog(4, `=== End DOM Structure Debug for "${lessonName}" ===`);
}
...
function extractStudyTime(container) {
    const timeSelectors = [
        ...
        '.lesson-content',
        '.content',
        // More specific yunxuetang selectors
        '.yxtulcdsdk-course-page__chapter-content *',
        '.yxtulcdsdk-course-player__chapter-content *',
        '.yxtulcdsdk-course-player__lesson-content *',
        // Look for elements with time-related classes
        '[class*="ml36"]',
        '[class*="ellipsis"]',
        // Check all text nodes for time patterns
        'span',
        'div',
        'p'
    ];
    
    debugLog(4, 'Extracting study time from container:', {
        containerClasses: container.className,
        containerChildren: container.children.length,
        containerHTML: container.innerHTML.substring(0, 200) + '...' // First 200 chars for debugging
    });
    ...
    for (const selector of timeSelectors) {
        try {
            ...
            if (timeElement) {
                ...
                const extractedTime = rawText.replace(/[^0-9.]/g, '').trim();
                debugLog(4, `Found study time with selector "${selector}":`, {
                    rawText: rawText,
            ...
            debugLog(4, `Selector "${selector}" failed:`, error);
        }
    }
    
    // If no specific selectors work, try to find any text containing time patterns
    debugLog(4, 'Trying to find time patterns in all text content...');
    const containerText = container.textContent;
    debugLog(4, 'All text content:', containerText.substring(0, 300) + '...');
    
    // Look for time patterns in the text
    const timePatterns = [
        /(\d+\.?\d*)\s*分钟/g,  // Chinese minutes
        /(\d+\.?\d*)\s*min/g,   // English minutes
        /(\d+\.?\d*)\s*小时/g,  // Chinese hours
        /(\d+\.?\d*)\s*h/g,     // English hours
        /(\d+\.?\d*)\s*秒/g,    // Chinese seconds
        /(\d+\.?\d*)\s*s/g,     // English seconds
        /(\d+):(\d+)/g,         // Time format like 10:30
        /(\d+\.?\d*)/g          // Any number
    ];
    ...
    for (const pattern of timePatterns) {
        ...
        if (matches && matches.length > 0) {
            debugLog(4, `Found time pattern "${pattern}":`, matches);
            // Return the first match that looks like a reasonable time
            for (const match of matches) {
                const time = match.replace(/[^0-9.]/g, '').trim();
                if (time && parseFloat(time) > 0) {
                    debugLog(4, `Extracted time from pattern: ${time}`);
                    return time;
                }
            }
        }
    ...
    // If still no time found, debug the container structure
    const lessonName = container.querySelector('[class*="title"], [class*="name"], [class*="text"]')?.textContent?.trim() || 'Unknown';
    debugContainerStructure(container, lessonName);
    
    // Final fallback: look for any reasonable numeric values
    debugLog(4, 'Trying final fallback: looking for any numeric values...');
    const fallbackText = container.textContent;
    const numbers = fallbackText.match(/\d+\.?\d*/g);
    if (numbers && numbers.length > 0) {
        debugLog(4, 'Found numeric values:', numbers);
        // Look for numbers that could be reasonable time values (between 0.1 and 999)
        for (const num of numbers) {
            const value = parseFloat(num);
            if (value >= 0.1 && value <= 999) {
                debugLog(4, `Using fallback time value: ${num}`);
                return num;
            }
        }
    }
    
    debugLog(4, 'No study time found with any method');
    return '';
}
...
export async function findAndClickLesson(lesson) {
    ...
    try {
        ...
        if (clicked) {
            // Hide tooltips in popup when lesson is clicked automatically
            try {
...
export function extractLessons() {
    try {
        ...
        lessonElements.forEach((container, index) => {
            ...
            debugLog(4, `Lesson ${index} "${name}": studyTime = "${studyTime}", type = "${type}"`);
